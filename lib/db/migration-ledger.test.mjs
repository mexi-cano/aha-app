...
test("PDF evidence migration serializes uploads and rejects integrity conflicts in-database", async () => {
  const sql = await readFile(
    new URL("./migrations/0001_vengeful_screwball.sql", import.meta.url),
    "utf8",
  );
  assert.match(sql, /CREATE TABLE "aha_pdf_revisions"/);
  assert.match(sql, /CREATE FUNCTION store_aha_pdf_version/);
  assert.match(sql, /pg_advisory_xact_lock/);
  assert.match(sql, /CONSTRAINT = 'aha_pdf_version_integrity'/);
