...
- **Late arrival (completed AHA):** "+ Add worker & sign" → name plus the same continuous worker review → acknowledgment → signature → PDF regenerates; existing signatures and completion time remain untouched.
...
3. Worker review: mid-signing, select a worker's "Review & sign ›" row → the full read-only AHA appears in natural page order without the crew roster → the official acknowledgment and signature canvas follow at the bottom → save returns to the roster with "✓ Signature saved" and focus on that worker. Nothing is editable en route, and the app makes no claim that scrolling proves the worker read every line.
