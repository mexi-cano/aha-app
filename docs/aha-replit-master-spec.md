...
- **Emergency contact quality:** Emergency number remains a required free-form safety instruction and may contain `911`, a site number, labels, extensions, international numbers, or compound directions. After an actual user edit and blur, the app formats only a standalone North American number as `(919) 555-0182` or `+1 (919) 555-0182`; `911` remains unchanged. Compound or ambiguous content is never rewritten, and merely opening, focusing, restoring, or blurring a legacy value changes nothing. A nonblank value without a recognizable `911`, North American, or explicit international number receives the advisory `Check that this includes the number the crew should call.` at the field and at Review; it does not block signing and cannot be marked Not applicable. Blank emergency information remains a blocker. No broad trimming, capitalization, or autocorrection applies to other user-entered safety text.
- **Location reuse:** Job setup may explicitly copy the trimmed permanent City or area into editable default Location with `USE CITY OR AREA`. Daily Details may explicitly restore the configured Location with `USE JOB DEFAULT`. These 48px actions appear only when their nonblank source differs, never maintain a live link, never overwrite typing automatically, and work fully offline through the existing local mutation path. Location remains free-form and may be a street address, intersection, station, subdivision, easement, or work area. Provider-backed address search, device geolocation, cross-job recent suggestions, and automatic emergency-centre selection are excluded from v1.

## 5. Validation — progressive; blocking is enforced only at Review

Field-level advisory feedback may appear after blur, but the editor never blocks typing or section navigation. Review remains the authoritative place for all signing blockers and unresolved warnings.
...
- **Warnings:** missing work order/permit, JHA/procedure numbers, and meeting notes each offer `Add` · `Not applicable`; "Not applicable" silences that warning and prints blank on the PDF (never print "N/A"). An unrecognized nonblank emergency contact offers `Check` only and cannot be marked Not applicable. Warnings never block signing.
- **Informational:** counts/confirmations. Warnings render amber with the ⚠ icon + text (never color alone, never red); each has an action that deep-links to the exact field.
